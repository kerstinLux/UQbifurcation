...
set(gca,'FontSize',12);
% savefig(fig1,'hist_X_div_1plusX_Uni46_LegBasis_N2_M10P6.fig');
% saveas(fig1,'hist_X_div_1plusX_Uni46_LegBasis_N2_M10P6.eps','epsc');
% savefig(fig1,'hist_X_div_1plusX_Beta25_LegBasis_N2_M10P6.fig');
% saveas(fig1,'hist_X_div_1plusX_Beta25_LegBasis_N2_M10P6.eps','epsc');
...
% %% Simulate Beta(alpha,beta)
% rng default
% alpha_val = 2;
% beta_val = 5;
% Theta = betarnd(alpha_val, beta_val,[M 1]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% %% for u \in [-1,1]
% for n=1:N+1
%     expPoly = expPoly + PCEcoeffs(n)*1/(sqrt(1/(2*(n-1)+1)))*legendreP(n-1,u);
% end
%% for u \in [0,1]
for n=1:N+1
...
% GMModel = fitgmdist(RV,n_Gauss,'Options',options);
...
% init_means = sum(supp_RV)/2*ones(1,n_Gauss);
% init_means = [0.05 0.35];
% init_means = [0.2 0.8];
% init_means = [0.01 0.1];
init_means = [-0.4 0.4];
% init_std = ones(1,n_Gauss);
% init_std = [0.01 0.05];
init_std = [0.01 0.1];
pars_init = [init_weights init_means init_std];
% pars_init = [1/12 3/4 1/6 0.1 1 2 0.1 0.5 1];
% pars_init = [1/3; 1/3; 1/3; 0.5*ones(2*n_Gauss,1)];
...
% [pars,qForm_val] = simulannealbnd(qForm,pars_init,[zeros(1,n_Gauss-1) -Inf*ones(1,n_Gauss) zeros(1,n_Gauss)],[ones(1,n_Gauss-1) Inf*ones(1,2*n_Gauss)],options);
pars_new = pars;
% % work with pseudo inverse as matrix is close to singular (pinv based on
% % SVD; idea from "Efficient GMM estimation with singular system of moment
% % conditions", Xiao (2019)
% W_new=pinv(mom_conds(pars)*mom_conds(pars)');
% qForm_new = @(pars) mom_conds(pars)'*W_new*mom_conds(pars);
% [pars_new,qForm_val_new] = fmincon(qForm_new,pars,[],[],[],[],[zeros(1,n_Gauss-1) -Inf*ones(1,n_Gauss) zeros(1,n_Gauss)],[ones(1,n_Gauss-1) Inf*ones(1,2*n_Gauss)],[],options);
...
% dens_Gmix = @(x) pars(1)*normpdf(x,pars(4),pars(7)) + pars(2)*normpdf(x,pars(5),pars(8)) + pars(3)*normpdf(x,pars(6),pars(9));
% cdf_Gmix = @(x) pars(1)*normcdf(x,pars(4),pars(7)) + pars(2)*normcdf(x,pars(5),pars(8)) + pars(3)*normcdf(x,pars(6),pars(9));

% Calculate MC CDF values
rangeLorenzRV = -0.4:0.001:0.3;
% rangeLorenzRV = -0.1:0.001:0.2;
% rangeLorenzRV = 0:0.001:0.4;
...
% mean_Gmix = pars(1)*pars(4)+pars(2)*pars(5)+pars(3)*pars(6);
% std_Gmix = pars(1)*pars(7)+pars(2)*pars(8)+pars(3)*pars(9);
mean_Gmix = 0;
for i=1:n_Gauss-1
...
leg = legend([barLorenzRV l1],{'Normalized histogram','Moment-based Gmix'});
leg.Interpreter = 'latex';
leg.FontSize= 12;
leg.Location = 'Northwest';
set(gca,'FontSize',12);
% savefig(fig2,'histVsPDFapprox_LorenzRV_Uni46_Gamma81_LegBasis_N2_numMoms7_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12_stopNondecr.fig');
% saveas(fig2,'histVsPDFapprox_LorenzRV_Uni46_Gamma81_LegBasis_N2_numMoms7_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12_stopNondecr.eps','epsc');
% savefig(fig2,'histVsPDFapprox_LorenzRV_Beta25_Gamma81_LegBasis_N2_numMoms5_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12.fig');
% saveas(fig2,'histVsPDFapprox_LorenzRV_Beta25_Gamma81_LegBasis_N2_numMoms5_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12.eps','epsc');
savefig(fig2,'histVsPDFapprox_LorenzRV_genBeta25_Min0K5_0K5_Gamma81_numMoms5.fig');
saveas(fig2,'histVsPDFapprox_LorenzRV_genBeta25_Min0K5_0K5_Gamma81_numMoms5.eps','epsc');
...
set(gca,'FontSize',12);
% savefig(fig3,'CDFapprox_LorenzRV_Uni46_Gamma81_LegBasis_N2_numMoms7_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12_stopNondecr.fig');
% saveas(fig3,'CDFapprox_LorenzRV_Uni46_Gamma81_LegBasis_N2_numMoms7_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12_stopNondecr.eps','epsc');
% savefig(fig3,'CDFapprox_LorenzRV_Beta25_Gamma81_LegBasis_N2_numMoms5_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12.fig');
% saveas(fig3,'CDFapprox_LorenzRV_Beta25_Gamma81_LegBasis_N2_numMoms5_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12.eps','epsc');
savefig(fig3,'CDFapprox_LorenzRV_genBeta25_suppMin0K5_0K5_Gamma81_numMoms5_stopnondecr_Teaser.fig');
saveas(fig3,'CDFapprox_LorenzRV_genBeta25_suppMin0K5_0K5_Gamma81_numMoms5_stopnondecr_Teaser.eps','epsc');

%% Calculate probability of supercritical Hopf bifurcation
y = [-0.15 -0.05 0 0.05];
% y = [0.01 0.02 0.05 0.1];
% y = [0 0.05 0.1 0.35];
...
myTable_prob = table(prob_bifCoeffLess,...
    'RowNames',MethodName)
% writetable(myTable_prob,'prob_LorenzRV_Beta25_Gamma81_LegBasis_N2_numMoms5_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12.xlsx','WriteRowNames',true);
% writetable(myTable_prob,'prob_LorenzRV_Uni46_Gamma81_LegBasis_N2_numMoms7_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12_stopNondecr.xlsx','WriteRowNames',true);
...
myTable_moms = table(calcMoms,...
    'RowNames',mom_method)
% writetable(myTable_moms,'moms_LorenzRV_Beta25_Gamma81_LegBasis_N2_numMoms5_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12.xlsx','WriteRowNames',true);
% writetable(myTable_moms,'moms_LorenzRV_Uni46_Gamma81_LegBasis_N2_numMoms7_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12_stopNondecr.xlsx','WriteRowNames',true);
...
myTable_pars = table(pars_opt,...
    'RowNames',parsComp)
% writetable(myTable_pars,'GMMpars_LorenzRV_Beta25_Gamma81_LegBasis_N2_numMoms5_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12.xlsx','WriteRowNames',true)
% writetable(myTable_pars,'GMMpars_LorenzRV_Uni46_Gamma81_LegBasis_N2_numMoms7_numComp2_Maxfeval1e+4_maxIter5mal1e+3_StepTol1e-12_stopNondecr.xlsx','WriteRowNames',true)
% writetable(myTable_pars,'prob_lyapCoeff_LorenzRV_n_Gauss5_numMoms14_N14_Widentity.xlsx','WriteRowNames',true)