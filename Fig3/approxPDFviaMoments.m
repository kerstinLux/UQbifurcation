...
switch PDF_method
    ...
    case 'monic'
        ...
        a = para.supp(1);
        b = para.supp(2);
        %% Calculate alpha,beta from B~gBeta(alpha,beta) s.th. c_w*E[B]=E[tN], c_w*E[B^2]=E[tN^2], where c_w = 1/integral(w,a,b) is a normalizing constant with
...
%         c_w_theo = 1/(beta(alpha,eta)*(b-a)^(1-alpha-eta));
        %% Calculate monic orthogonal polynomial coefficients
        % Calculate moments of correponding Beta distribution
        paraMonic = para;
%         paraMonic.alpha = (moms(2)-a)*(moms(3)+a*b - (a+b)*moms(2))/((b-a)*(moms(2)^2-moms(3)));
%         paraMonic.beta = (b-moms(2))*(moms(3)+a*b - (a+b)*moms(2))/((b-a)*(moms(2)^2-moms(3)));
...
%         D_Hankelmod = zeros(N_trunc+1,1);
        ...
        %% Coefficients of pi_i(x) do now coincide with example 3.1 from Provost(2015)
    ...
    case 'transformedMoments'
        %% Calculate moment-based pdf approximation according to "Hausdorff moment problem: Reconstruction of distributions", Mnatsakanov(2008), formula (2), "Hausdorff moment problem: Reconstruction of PDFs", Mnatsakanov(2008b), formula (6)
